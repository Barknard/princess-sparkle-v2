...
function generateHouse(rng, wantChimney) {
  ...
  const w = 3 + Math.floor(rng() * 4);   // 3-6 wide (min 3 so door isn't cut off)
  const rows = [];

  // Helper: make a row of L-M-R tiles (w >= 3 guaranteed)
  const makeRow = (lmr) => {
    const row = [lmr.L];
  ...
  // Pick door position (center-ish, never at edges)
  const doorX = 1 + Math.floor(rng() * (w - 2));
...
class V2Engine {
  ...
  generate(dna = {}, seed) {
    ...
    // Scale cluster count and size to map dimensions (works for both 24x14 and 60x40)
    ...
    for (let ci = 0; ci < numClusters; ci++) {
      let cx, cy;
      if (ci < Math.min(4, numClusters - 1)) {
        // First clusters: always corners
        const corner = cornerPositions[ci % 4];
        cx = corner.cx + Math.floor(rng() * Math.min(3, Math.floor(this.W / 8)));
        cy = corner.cy + Math.floor(rng() * Math.min(2, Math.floor(this.H / 7)));
      } else if (rng() < 0.75) {
        // Remaining clusters: edge positions, scaled to map
        const edgeDepth = isSmallMap ? 3 : 5;
        const edge = Math.floor(rng() * 4);
        if (edge === 0) { cx = Math.floor(rng() * edgeDepth); cy = Math.floor(rng() * this.H); }
        else if (edge === 1) { cx = this.W - edgeDepth + Math.floor(rng() * edgeDepth); cy = Math.floor(rng() * this.H); }
        else if (edge === 2) { cx = Math.floor(rng() * this.W); cy = Math.floor(rng() * edgeDepth); }
        else { cx = Math.floor(rng() * this.W); cy = this.H - edgeDepth + Math.floor(rng() * edgeDepth); }
      } else {
        // Random interior (for scattered variety)
        const insetX = isSmallMap ? 3 : 5;
        const insetY = isSmallMap ? 2 : 4;
      ...
      const cw = 2 + Math.floor(rng() * maxClusterW);  // scaled cluster width
      const ch = 2 + Math.floor(rng() * maxClusterH);  // scaled cluster height
      // Ensure cluster fits within map bounds
      const clampedW = Math.min(cw, this.W - cx);
      const clampedH = Math.min(ch, this.H - cy);
      if (clampedW < 2 || clampedH < 2) continue;
      placeTreeCluster(cx, cy, clampedW, clampedH);
      clusterZones.push({ x: cx, y: cy, w: clampedW, h: clampedH });
    }
    ...
    for (let attempt = 0; attempt < singleCount * 8 && singlesPlaced < singleCount; attempt++) {
      ...
      // Pick tree type with best adjacency + cross-layer match (try 3 candidates)
      const scoreTree = (tt) => {
        let s = scoreFgAdjacency(x, y, tt.canopy) + scoreFgAdjacency(x, y + 1, tt.trunk);
        // Cross-layer: has this tree been seen on this ground type?
        if (this._columnPatterns) {
          const gTile = ground[this.idx(x, y + 1)]; // trunk's ground matters most
          const trunkKey = gTile + ',-1,' + tt.trunk;
          if (this._columnPatterns[trunkKey]) s += 0.3;
        }
        return s;
      };
      let bestTree = singlePool[Math.floor(rng() * singlePool.length)];
      let bestAdj = scoreTree(bestTree);
      for (let c = 0; c < 2; c++) {
        const cand = singlePool[Math.floor(rng() * singlePool.length)];
        const adjS = scoreTree(cand);
        if (adjS > bestAdj) { bestTree = cand; bestAdj = adjS; }
      }
      if (placeTree(x, y, bestTree.canopy, bestTree.trunk)) singlesPlaced++;
    }
    ...
    const EDGE_TO_BODY_DIR = {
      // dense1_A: top(7)→south, bottom(31)→north, left(18)→east, right(20)→west
      7: [0, 1], 10: [0, 1],     // top edges expect body below
      31: [0, -1], 34: [0, -1],  // bottom edges expect body above
      18: [1, 0], 21: [1, 0],    // left edges expect body to the right
      20: [-1, 0], 23: [-1, 0],  // right edges expect body to the left
    };
    for (let y = 0; y < this.H; y++) {
      for (let x = 0; x < this.W; x++) {
        const ci = this.idx(x, y);
        ...
        // Body must be a cluster center tile (19=dense1_A, 22=dense1_B)
        if (bodyTile !== 19 && bodyTile !== 22) {
    ...
    const targetFg = Math.round(this.size * (d.fgDensity || Math.min(0.12, templateFgDensity)));
    let currentFg = 0;
    ...
    const standaloneFg = new Set([28, 17, 27]); // only true standalone bushes (not cluster edges or tree parts)
    let fgScatterPool = [];
    if (this._targetFreqs && this._targetFreqs.foreground) {
      for (const [tileStr, freq] of Object.entries(this._targetFreqs.foreground)) {
        ...
        if (standaloneFg.has(tile)) {
          // Weight by frequency: more common tiles appear more in pool
          const count = Math.max(1, Math.round(freq * 100));
          for (let i = 0; i < count; i++) fgScatterPool.push(tile);
        }
      }
    }
    if (fgScatterPool.length === 0) fgScatterPool = [28, 20, 32, 17, 27, 31, 34]; // fallback
    for (let attempt = 0; attempt < (targetFg - currentFg) * 4 && currentFg < targetFg; attempt++) {
      ...
      // Try 3 candidates and pick the best by combined adjacency + cross-layer score
      const scoreCandidate = (tile) => {
        let s = scoreFgAdjacency(x, y, tile);
        // Cross-layer bonus: has this (ground, objects, fg) column been seen in painted map?
        if (this._columnPatterns) {
          const gTile = ground[this.idx(x, y)];
          const oTile = objects[this.idx(x, y)];
          const key = gTile + ',' + oTile + ',' + tile;
          if (this._columnPatterns[key]) {
            s += 0.5 * Math.min(1, this._columnPatterns[key] / 3); // bonus scales with frequency
          }
        }
        return s;
      };
      let bestTile = fgScatterPool[Math.floor(rng() * fgScatterPool.length)];
      let bestAdj = scoreCandidate(bestTile);
      for (let c = 0; c < 2; c++) {
        const candidate = fgScatterPool[Math.floor(rng() * fgScatterPool.length)];
        const adjScore = scoreCandidate(candidate);
        if (adjScore > bestAdj) { bestTile = candidate; bestAdj = adjScore; }
      }
    ...
    for (const door of doorPositions) {
      if (rng() < 0.5) {
        ...
        if (this.inBounds(dx, dy) && objects[this.idx(dx, dy)] === T.EMPTY && !buildingBuffer.has(this.idx(dx, dy))) {
          // Cross-layer: prefer decorations on ground tiles they've been seen on
          const decoTile = rng() < 0.7 ? DECORATION_TILES.barrel : DECORATION_TILES.lantern;
          if (this._columnPatterns) {
            const gTile = ground[this.idx(dx, dy)];
            const key = gTile + ',' + decoTile + ',-1';
            // Skip if this ground+deco combo was never seen (likely wrong)
            if (!this._columnPatterns[key] && rng() < 0.5) continue;
          }
          objects[this.idx(dx, dy)] = decoTile;
